...
import { api } from "../api";
import { PageHeader, Surface } from "../components/workspace";
...
export default function Warming() {
  ...
  const activeWarmings = warmings.filter((warming) => warming.status === "running").length;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="Warming"
        description="Account warm-up, maintenance actions, channel pool hygiene, and observable A/B groups for long-lived Telegram accounts."
        stats={[
          { label: "Active warmings", value: activeWarmings, tone: activeWarmings ? "emerald" : "neutral", caption: "Currently running" },
          { label: "Profiles", value: profiles.length, tone: profiles.length ? "blue" : "neutral", caption: "Reusable warm-up presets" },
          { label: "Pool channels", value: pool.length, tone: pool.length ? "violet" : "neutral", caption: "Candidate subscription sources" },
        ]}
      />

      <Surface className="w-fit p-1">
        {TABS.map(t => (
